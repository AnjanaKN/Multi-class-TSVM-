...
{ 
  ...
  for(int i=0;i<Outputs->d;i++)
  { if (TrueLabels->vec[i]<-5)
      continue;
    if (Outputs->vec[i]*TrueLabels->vec[i]>0)
      {if(Outputs->vec[i]<0)
        tn +=1;
      else tp +=1;
      }
  ...
  cout<<tp<<" tn: "<<tn<<" fp: "<<fp<<" fn: "<<fn<<endl;
  double precision = 1.0*tp/(fp+tp);
  double recall=1.0*tp/(tp+fn);
  double guess = 1.0*(tn+tp)/(tn+tp+fn+fp);
  cout << "Accuracy = " << accuracy*100.0/Outputs->d << " %" << guess<<endl;
  cout<<"Precision = "<<precision<<endl;
  cout<<"Recall = "<<recall<<endl;
  cout<<"F1 = "<<1*precision*recall/(precision+recall)<<endl;
}
